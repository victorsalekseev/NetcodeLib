...
#region Класс AcedCast5
/*
--------------------------------------------------------------------------------
public sealed class AcedCast5

  Предназначен для шифрования и дешифрования массива байт методом CAST-128
  в режиме CFB (64 бита). Ключ шифра передается в методы данного класса как
  параметр keyBytes, представляющий собой массив из 16 байт. В методы для
  поточного шифрования, которые принимают параметр key, следует передавать
  не ключ шифра, а специальный массив, возвращаемый функцией ScheduleKey.
  Этот массив создается на основе ключа шифра keyBytes. При использовании
  в многопоточном приложении необходимо синхронизировать вызов методов
  данного класса.

--------------------------------------------------------------------------------
Методы для поточного шифрования данных
--------------------------------------------------------------------------------

public static int[] ScheduleKey(byte[] keyBytes)

  Создает массив, содержащий информацию о ключе шифра keyBytes. Размер ключа -
  16 байт. Функция возвращает массив, который может затем передаваться
  в качестве параметре key в другие методы данного класса, а в конце должен
  очищаться вызовом метода ClearKey().

public static void ScheduleKey(byte[] keyBytes, int[] key)

  Метод, аналогичный предыдущей функции, но здесь предполагается, что массив
  key был создан ранее вызовом функции ScheduleKey и его можно использовать
  повторно, заполнив информацией о новом ключе шифра. Ключ шифра размером
  16 байт передается параметром keyBytes. Данные о нем сохраняются в массиве
  key, который затем может передаваться в другие методы данного класса, а
  в конце должен очищаться вызовом метода ClearKey(). 

public static long GetOrdinaryIV(int[] key)

  Возвращает значение типа System.Int64, которое может использоваться
  в качестве начального вектора iv при вызове методов Encrypt и Decrypt.
  В параметре key передается массив, полученный из функции ScheduleKey().

public static long Encrypt(int[] key, byte* bytes, int length, long iv)

  Шифрует массив байт bytes длиной length байт. В параметре key передается
  массив, созданный функцией ScheduleKey(), в параметре iv - начальный вектор.
  Функция возвращает значение типа System.Int64, которое может использоваться
  как начальный вектор при шифровании следующего фрагмента данных.

public static long Encrypt(int[] key, byte[] bytes,
  int offset, int length, long iv)

  Шифрует фрагмент массива байт bytes, начиная со смещения offset, длиной
  length байт. В параметре key передается массив, созданный функцией
  ScheduleKey(), в параметре iv - начальный вектор. Функция возвращает значение
  типа System.Int64, которое может использоваться как начальный вектор при
  шифровании следующего фрагмента данных.

public static long Decrypt(int[] key, byte* bytes, int length, long iv)

  Дешифрует массив байт bytes длиной length байт, зашифрованный методом
  Encrypt. В параметре key передается массив, созданный функцией ScheduleKey(),
  в параметре iv - начальный вектор. Функция возвращает значение типа
  System.Int64, которое может использоваться как начальный вектор при
  дешифровании следующего фрагмента данных.

public static long Decrypt(int[] key, byte[] bytes,
  int offset, int length, long iv)

  Дешифрует фрагмент массива байт bytes, начиная со смещения offset, длиной
  length байт, зашифрованный методом Encrypt. В параметре key передается
  массив, созданный функцией ScheduleKey(), в параметре iv - начальный вектор.
  Функция возвращает значение типа System.Int64, которое может использоваться
  как начальный вектор при дешифровании следующего фрагмента данных.

public static void ClearKey(int[] key)

  Очищает массив key, содержащий ключевую информацию, используемую при
  шифровании данных. Предполагается, что этот массив был сгенерирован и
  заполнен функцией ScheduleKey().

--------------------------------------------------------------------------------
Методы для шифрования одного фрагмента данных
--------------------------------------------------------------------------------

public static int[] Encrypt(byte[] keyBytes, byte* bytes, int length)

  Шифрует массив, адресуемый параметром bytes, длиной length байт. Ключ
  шифра keyBytes передается в виде массива из 16 байт. Функция возвращает
  цифровую сигнатуру RipeMD-160 в виде массива из 5 элементов типа
  System.Int32, рассчитанную для массива bytes до того, как он был зашифрован.
  В качестве начального вектора используется значение функции GetOrdinaryIV(),
  рассчитанное относительно ключа keyBytes.

public static int[] Encrypt(byte[] keyBytes, byte[] bytes,
  int offset, int length)

  Шифрует фрагмент массива bytes, начиная со смещения offset, длиной length
  байт. Ключ шифра keyBytes передается в виде массива из 16 байт. Функция
  возвращает цифровую сигнатуру RipeMD-160 в виде массива из 5 элементов типа
  System.Int32, рассчитанную для шифруемого фрагмента до того, как он был
  зашифрован. В качестве начального вектора используется значение функции
  GetOrdinaryIV(), рассчитанное относительно ключа keyBytes.

public static int[] Encrypt(byte[] keyBytes, byte* bytes,
  int length, ref long iv)

  Шифрует массив, адресуемый параметром bytes, длиной length байт. Ключ шифра
  keyBytes передается в виде массива из 16 байт. Функция возвращает цифровую
  сигнатуру RipeMD-160 в виде массива из 5 элементов типа System.Int32,
  рассчитанную для массива bytes до того, как он был зашифрован. Начальный
  вектор передается параметром iv. Тот же параметр используется для возврата
  нового значения вектора iv в вызывающую функцию.

public static int[] Encrypt(byte[] keyBytes, byte[] bytes,
  int offset, int length, ref long iv)

  Шифрует фрагмент массива bytes, начиная со смещения offset, длиной length
  байт. Ключ шифра keyBytes передается в виде массива из 16 байт. Функция
  возвращает цифровую сигнатуру RipeMD-160 в виде массива из 5 элементов типа
  System.Int32, рассчитанную для массива bytes до того, как он был зашифрован.
  Начальный вектор передается параметром iv. Тот же параметр используется
  для возврата нового значения вектора iv в вызывающую функцию.

public static int[] Decrypt(byte[] keyBytes, byte* bytes, int length)

  Расшифровывает массив, адресуемый параметром bytes, длиной length байт,
  зашифрованный с ключом keyBytes. Ключ передается в виде массива из 16 байт.
  Функция возвращает цифровую сигнатуру RipeMD-160 в виде массива из 5
  элементов типа System.Int32, рассчитанную для массива bytes после того, как
  он расшифрован. В качестве начального вектора используется значение функции
  GetOrdinaryIV(), рассчитанное относительно ключа keyBytes.

public static int[] Decrypt(byte[] keyBytes, byte[] bytes,
  int offset, int length)

  Расшифровывает фрагмент массива bytes, начиная со смещения offset, длиной
  length байт, зашифрованный с ключом keyBytes. Ключ передается в виде
  массива из 16 байт. Функция возвращает цифровую сигнатуру RipeMD-160 в виде
  массива из 5 элементов типа System.Int32, рассчитанную для массива bytes
  после того, как он расшифрован. В качестве начального вектора используется
  значение функции GetOrdinaryIV(), рассчитанное относительно ключа keyBytes.
...
public static int[] Decrypt(byte[] keyBytes, byte* bytes,
  ...
  Функция возвращает цифровую сигнатуру RipeMD-160 в виде массива из 5
  элементов типа System.Int32, рассчитанную для массива bytes после того, как
  он расшифрован. Начальный вектор передается параметром iv. Тот же параметр
  используется для возврата нового значения вектора iv в вызывающую функцию.
...
public static int[] Decrypt(byte[] keyBytes, byte[] bytes,
  ...
  массива из 16 байт. Функция возвращает цифровую сигнатуру RipeMD-160 в виде
  массива из 5 элементов типа System.Int32, рассчитанную для массива bytes
  после того, как он расшифрован. Начальный вектор передается параметром iv.
  Тот же параметр используется для возврата нового значения вектора iv в
  вызывающую функцию.
    ...
    public sealed class AcedCast5
    {
        ...
        private static unsafe void IntScheduleKey(byte[] keyBytes, uint* K)
        {
            ...
            fixed (uint* S5 = &S5_SBox[0], S6 = &S6_SBox[0], S7 = &S7_SBox[0], S8 = &S8_SBox[0])
            {
                ...
                {
                    a = ((uint)p[0] << 24) | ((uint)p[1] << 16) | ((uint)p[2] << 8) | (uint)p[3];
                    b = ((uint)p[4] << 24) | ((uint)p[5] << 16) | ((uint)p[6] << 8) | (uint)p[7];
                    c = ((uint)p[8] << 24) | ((uint)p[9] << 16) | ((uint)p[10] << 8) | (uint)p[11];
                    d = ((uint)p[12] << 24) | ((uint)p[13] << 16) | ((uint)p[14] << 8) | (uint)p[15];
                }
        ...
        [CLSCompliant(false)]
        public static unsafe long Encrypt(int[] key, byte* bytes, int length, long iv)
        {
            fixed (int* K = &key[0])
            fixed (uint* pS1 = &S1_SBox[0], pS2 = &S2_SBox[0], pS3 = &S3_SBox[0], pS4 = &S4_SBox[0])
            {
                ...
                if (length > 0)
                {
                    ...
                    switch (length)
                    {
                        case 1:
                            ...
                            break;
                        case 2:
                            L ^= (uint)bytes[0] | ((uint)bytes[1] << 8);
                            bytes[0] = (byte)L;
                            bytes[1] = (byte)(L >> 8);
                            break;
                        case 3:
                            L ^= (uint)bytes[0] | ((uint)bytes[1] << 8) | ((uint)bytes[2] << 16);
                            bytes[0] = (byte)L;
                            bytes[1] = (byte)(L >> 8);
                            bytes[2] = (byte)(L >> 16);
                            break;
                        ...
                        case 7:
                            L ^= *((uint*)bytes);
                            R ^= (uint)bytes[4] | ((uint)bytes[5] << 8) | ((uint)bytes[6] << 16);
                            *((uint*)bytes) = L;
                            bytes[4] = (byte)R;
                            bytes[5] = (byte)(R >> 8);
                            bytes[6] = (byte)(R >> 16);
        ...
        public static unsafe long Encrypt(int[] key, byte[] bytes, int offset, int length, long iv)
        {
            if (length > 0)
                fixed (byte* pBytes = &bytes[offset])
                    return Encrypt(key, pBytes, length, iv);
            return iv;
        }

        [CLSCompliant(false)]
        public static unsafe long Decrypt(int[] key, byte* bytes, int length, long iv)
        {
            fixed (int* K = &key[0])
            fixed (uint* pS1 = &S1_SBox[0], pS2 = &S2_SBox[0], pS3 = &S3_SBox[0], pS4 = &S4_SBox[0])
            {
                ...
                if (byteCount > 0)
                {
                    ...
                    {
                        case 1:
                            Lnext = (L & 0xFFFFFF00u) | bytes[0];
                            Rnext = R;
                            bytes[0] ^= (byte)L;
                            break;
                        case 2:
                            Lnext = (L & 0xFFFF0000u) | (uint)bytes[0] | ((uint)bytes[1] << 8);
                            Rnext = R;
                            bytes[0] ^= (byte)L;
                            bytes[1] ^= (byte)(L >> 8);
                            break;
                        case 3:
                            Lnext = (L & 0xFF000000u) | (uint)bytes[0] | ((uint)bytes[1] << 8) | ((uint)bytes[2] << 16);
                            Rnext = R;
                            bytes[0] ^= (byte)L;
                            bytes[1] ^= (byte)(L >> 8);
                            bytes[2] ^= (byte)(L >> 16);
                        ...
                        case 5:
                            Lnext = *((uint*)bytes);
                            Rnext = (R & 0xFFFFFF00u) | bytes[4];
                            *((uint*)bytes) = Lnext ^ L;
                            bytes[4] ^= (byte)R;
                            break;
                        case 6:
                            Lnext = *((uint*)bytes);
                            Rnext = (R & 0xFFFF0000u) | (uint)bytes[4] | ((uint)bytes[5] << 8);
                            *((uint*)bytes) = Lnext ^ L;
                            bytes[4] ^= (byte)R;
                            bytes[5] ^= (byte)(R >> 8);
                            break;
                        case 7:
                            Lnext = *((uint*)bytes);
                            Rnext = (R & 0xFF000000u) | (uint)bytes[4] | ((uint)bytes[5] << 8) | ((uint)bytes[6] << 16);
                            *((uint*)bytes) = Lnext ^ L;
                            bytes[4] ^= (byte)R;
                            bytes[5] ^= (byte)(R >> 8);
                            bytes[6] ^= (byte)(R >> 16);
        ...
        public static unsafe long Decrypt(int[] key, byte[] bytes, int offset, int length, long iv)
        {
            if (length > 0)
                fixed (byte* pBytes = &bytes[offset])
                    return Decrypt(key, pBytes, length, iv);
            return iv;
        }

        public static unsafe void ClearKey(int[] key)
        {
            fixed (int* K = &key[0])
                AcedBinary.Fill(0, K, 32);
        }

        [CLSCompliant(false)]
        public static unsafe int[] Encrypt(byte[] keyBytes, byte* bytes, int length)
        {
            int[] result = AcedRipeMD.Compute(bytes, length);
            int[] key = ScheduleKey(keyBytes);
            Encrypt(key, bytes, length, GetOrdinaryIV(key));
            ClearKey(key);
            return result;
        }

        public static unsafe int[] Encrypt(byte[] keyBytes, byte[] bytes, int offset, int length)
        {
            if (length > 0)
                fixed (byte* pBytes = &bytes[offset])
                    return Encrypt(keyBytes, pBytes, length);
            return AcedRipeMD.NoDataHash();
        }

        [CLSCompliant(false)]
        public static unsafe int[] Encrypt(byte[] keyBytes, byte* bytes, int length, ref long iv)
        {
            int[] result = AcedRipeMD.Compute(bytes, length);
            int[] key = ScheduleKey(keyBytes);
            iv = Encrypt(key, bytes, length, iv);
            ClearKey(key);
            return result;
        }

        public static unsafe int[] Encrypt(byte[] keyBytes, byte[] bytes, int offset, int length, ref long iv)
        {
            if (length > 0)
                fixed (byte* pBytes = &bytes[offset])
                    return Encrypt(keyBytes, pBytes, length, ref iv);
            return AcedRipeMD.NoDataHash();
        }

        [CLSCompliant(false)]
        public static unsafe int[] Decrypt(byte[] keyBytes, byte* bytes, int length)
        {
            int[] key = ScheduleKey(keyBytes);
            Decrypt(key, bytes, length, GetOrdinaryIV(key));
            ClearKey(key);
            return AcedRipeMD.Compute(bytes, length);
        }

        public static unsafe int[] Decrypt(byte[] keyBytes, byte[] bytes, int offset, int length)
        {
            if (length > 0)
                fixed (byte* pBytes = &bytes[offset])
                    return Decrypt(keyBytes, pBytes, length);
            return AcedRipeMD.NoDataHash();
        }

        [CLSCompliant(false)]
        public static unsafe int[] Decrypt(byte[] keyBytes, byte* bytes, int length, ref long iv)
        {
            int[] key = ScheduleKey(keyBytes);
            iv = Decrypt(key, bytes, length, iv);
            ClearKey(key);
            return AcedRipeMD.Compute(bytes, length);
        }

        public static unsafe int[] Decrypt(byte[] keyBytes, byte[] bytes, int offset, int length, ref long iv)
        {
            if (length > 0)
                fixed (byte* pBytes = &bytes[offset])
                    return Decrypt(keyBytes, pBytes, length, ref iv);
            return AcedRipeMD.NoDataHash();
        }
