...
using Netcode.File.Archive.SharpZipLib.Checksums;
using Netcode.File.Archive.SharpZipLib.Zip.Compression.Streams;
using Netcode.File.Archive.SharpZipLib.Zip.Compression;
using Netcode.File.Archive.SharpZipLib.Encryption;
...
namespace Netcode.File.Archive.SharpZipLib.Zip 
{
	...
	public class ZipFile : IEnumerable
	{
		...
		{
			this.name = name;
            this.baseStream = System.IO.File.OpenRead(name);
			try {
				ReadEntries();
			}
			catch {
...
*/
		...
		Stream GetOutputStream(ZipEntry entry, string fileName)
		{
            ...
            Stream result = System.IO.File.OpenWrite(fileName);
