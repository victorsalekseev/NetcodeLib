...
namespace Netcode.File.Archive.SharpZipLib.Tar
{
	/// <summary>
		...
		void ExtractEntry(string destDir, TarEntry entry)
		{
			...
			if (entry.IsDirectory) {
				...
				if (process) {
                    ...
                    Stream outputStream = System.IO.File.Create(destFile);
		...
		void InternalWriteEntry(TarEntry sourceEntry, bool recurse)
		{
			...
			if (this.asciiTranslate && !entry.IsDirectory) {
				...
				if (asciiTrans) {
                    ...
                    StreamReader inStream  = System.IO.File.OpenText(entryFilename);
                    Stream       outStream = System.IO.File.Create(tempFileName);
			...
			if (entry.IsDirectory) {
                ...
                Stream inputStream = System.IO.File.OpenRead(entryFilename);
				int numWritten = 0;
